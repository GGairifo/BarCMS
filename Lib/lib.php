...
function isValidTP($userName, $password) {
    dbConnect(ConfigFile);
    $dataBaseName = $GLOBALS['configDataBase']->db;
    mysqli_select_db($GLOBALS['ligacao'], $dataBaseName);

    $query = "SELECT * FROM `$dataBaseName`.`utilizador` WHERE `user_name` = ?";
    $stmt = mysqli_prepare($GLOBALS['ligacao'], $query);
    if (!$stmt) {
        echo "Erro na preparação da consulta: " . mysqli_error($GLOBALS['ligacao']) . "<br>";
        dbDisconnect();
        exit();
    }
    ...
    if ($result) {
        if (mysqli_num_rows($result) > 0) {
            ...
            echo "Debug: Password from DB (MD5) - $md5PasswordFromDb<br>";

            if ($isActive == 0) {
                echo "Account needs activation.<br>";
                dbDisconnect();
                return -1;
            }

            // Verificação da senha em texto simples e MD5
            if ($password === $md5PasswordFromDb || substr(md5($password), 0, 9) === $md5PasswordFromDb) {
                echo "Debug: Password verification successful<br>";
                $userOk = $userData['utilizador_id'];
            } else {
        ...
        echo "Erro na consulta: " . mysqli_error($GLOBALS['ligacao']) . "<br>";
        dbDisconnect();
        return -1;
    }
    mysqli_free_result($result);
    dbDisconnect();
    return isset($userOk) ? $userOk : -1;
}
...
function Registo($userName, $email, $password, $authType) {
    dbConnect(ConfigFile);
    $dataBaseName = $GLOBALS['configDataBase']->db;
    mysqli_select_db($GLOBALS['ligacao'], $dataBaseName);
    $query = "INSERT * INTO `$dataBaseName`.`$authType` WHERE `user_name`='$userName' AND `password`='$password'";
    $result = mysqli_query($GLOBALS['ligacao'], $query);
    
    if ($result) {
        if(mysqli_num_rows($result) > 0) {
            $userData = mysqli_fetch_array($result);
            $userOk = $userData['utilizador_id'];  // Confirme o nome correto da coluna de ID.
        } else {
            echo "Nenhum usuário encontrado.";
        }
    } else {
        echo "Erro na consulta: " . mysqli_error($GLOBALS['ligacao']);
    }
    mysqli_free_result($result);
    dbDisconnect();
    return isset($userOk) ? $userOk : -1;
